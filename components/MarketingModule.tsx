...
import { Image as ImageIcon, Star, Trash2, Plus, Upload, Camera, Lock, Loader2, Award, Sparkles, MessageSquare, ExternalLink, X } from 'lucide-react';
...
export const MarketingModule: React.FC<MarketingModuleProps> = ({ 
    portfolio, onAddPortfolioItem, onDeletePortfolioItem, reviews, currentPlan, onUpgrade, businessId 
}) => {
    ...
    const handleAddImage = async () => {
        ...
        try {
            ...
            if (uploadError) {
                console.error("Storage Detalhado:", uploadError);
            ...
            alert(`Erro no Storage: ${storageErr.message || "Falha ao enviar arquivo."}`);
            return; // Interrompe o fluxo se falhar no storage
        }

        // BLOCO 2: Insert no Banco
        try {
            const payload = {
            ...
            if (dbError) {
                console.error("Banco Detalhado:", dbError);
            ...
            alert(`Erro no Banco: ${dbErr.message || "Falha ao salvar metadados."}`);
        } finally {
            setIsUploading(false);
        }
    };
        ...
        <div className="p-6 md:p-12 pb-24 space-y-12 max-w-[1800px] mx-auto animate-fade-in">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-4 gap-10">
                    ...
                    <div className="bg-[#09090b] border border-white/10 rounded-[2.5rem] p-8 h-fit space-y-6 relative overflow-hidden group hover:border-white/20 transition-all shadow-2xl">
                        ...
                        <div 
                            ...
                            className={cn(
                                "aspect-square rounded-[2rem] flex flex-col items-center justify-center transition-all relative overflow-hidden group/upload",
                                previewUrl ? "cursor-default" : "cursor-pointer border-2 border-dashed border-zinc-800 bg-black/40 hover:border-red-600/30 hover:bg-red-900/5"
                            )}
                        >
                            ...
                            {previewUrl ? (
                                ...
                                <div className="text-center p-6">
                                    <div className="w-16 h-16 rounded-2xl bg-zinc-900 border border-white/5 flex items-center justify-center mx-auto mb-4 group-hover/upload:scale-110 transition-transform shadow-lg">
                                         <ImageIcon className="text-zinc-600 group-hover/upload:text-red-500 transition-colors" size={24} />
                                    </div>
                        ...
                        <div className="relative z-10">
                            <input 
                                className="w-full bg-zinc-950/80 border border-white/5 rounded-xl px-4 py-4 text-xs font-bold text-white uppercase outline-none focus:border-red-600/50 transition-all placeholder:text-zinc-700" 
                        ...
                        <button 
                            ...
                            disabled={!previewUrl || !desc || isUploading} 
                            className={cn(
                                "w-full py-4 font-black uppercase tracking-widest rounded-xl shadow-lg flex items-center justify-center gap-3 transition-all relative z-10 active:scale-95",
                                (!previewUrl || !desc || isUploading) 
                            ...
                            <div key={item.id} className="relative aspect-[4/5] rounded-[2.5rem] overflow-hidden bg-zinc-950 border border-white/5 group shadow-2xl transition-transform hover:-translate-y-2 duration-500">
                                ...
                                <div className="absolute bottom-0 inset-x-0 p-8 z-20 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">Hangar CarbonCar</span>
                                    </div>
                                ...
                                <button onClick={() => onDeletePortfolioItem(item.id)} className="absolute top-6 right-6 p-3 bg-black/40 backdrop-blur-md border border-white/10 rounded-full text-white/50 opacity-0 group-hover:opacity-100 transition-all hover:bg-red-600 hover:text-white hover:border-red-500 z-30 transform hover:scale-110">
                                    <Trash2 size={16} />
                                </button>
                            </div>
                        ))}
                        {portfolio.length === 0 && (
                            <div className="col-span-full h-96 flex flex-col items-center justify-center border border-white/5 rounded-[3rem] bg-zinc-900/20">
                                <ImageIcon size={48} className="text-zinc-800 mb-4" />
                                <p className="text-xs font-black uppercase text-zinc-600 tracking-[0.3em]">Nenhuma foto no showroom</p>
                            </div>
                        )}
                    </div>
                </div>
            ...
            {activeTab === 'reviews' && (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {reviews.length === 0 ? (<div className="col-span-full py-32 text-center opacity-30"><MessageSquare size={48} className="mx-auto mb-4" /><p className="text-xs font-black uppercase tracking-[0.3em]">Aguardando feedbacks</p></div>) : (
                        reviews.map(rev => (<div key={rev.id} className="bg-[#09090b] border border-white/10 p-8 rounded-[2.5rem] space-y-4">
                            <div className="flex justify-between items-start"><div><p className="text-sm font-black text-white uppercase">{rev.customerName}</p></div></div>
